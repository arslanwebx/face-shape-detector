...
export function LogoIcon({ className = "" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 48 48" role="img" aria-label="Face Shape Detector mark">
...
export default function Logo({ compact = false, inverse = false }: { compact?: boolean; inverse?: boolean }) {
  return (
    <Link className={`brand${compact ? " brand-compact" : ""}${inverse ? " brand-inverse" : ""}`} href="/" aria-label={`${siteConfig.brandName} home`}>
      ...
      <span className="brand-wordmark" aria-hidden="true"><strong>Face Shape Detector</strong>{!compact && <span>Face shape analysis</span>}</span>
    </Link>
  );
}
